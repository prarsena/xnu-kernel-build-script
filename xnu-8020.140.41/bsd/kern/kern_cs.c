...
/*
 * csvnode_invalidate_flags: clear CS_VALID from all code-signing blobs
 * attached to a vnode.
 *
 * Stub for symbol omitted from the open-source drop but required by the
 * binary AppleMobileFileIntegrity kext.  cs_blob structs live in a
 * read-only zone, so mutations must go through zalloc_ro_update_field.
 */
void
csvnode_invalidate_flags(struct vnode *vp)
{
	...
	for (blob = ubc_get_cs_blobs(vp); blob != NULL; blob = blob->csb_next) {
		new_flags = blob->csb_flags & ~CS_VALID;
		printf("csvnode_invalidate_flags: old 0x%x new 0x%x\n",
		    blob->csb_flags, new_flags);
		zalloc_ro_update_field(ZONE_ID_CS_BLOB, blob, csb_flags, &new_flags);
	}
}
